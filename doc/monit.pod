...
Some platforms allows to differentiate the I/O that required physical
storage access. The physical I/O activity is usually aligned to the
filesystem page size, so the total and physical I/O may differ even
if all operations required physical storage access.
