...
Monit can test the number of open files of a process.
You can check the number of open files for a single process, or the
accumulated number for the process and all its children.

The syntax for the open-files statement is:
...
Example:
 ...
 check process myproc with pidfile /var/run/myproc.pid
    if open-files >= 100 then alert
    if total open-files > 500 then restart
