...
=head2 PROCESS OPEN FILES TEST

Monit can test the number of open files of a process. You can check either
an absolute value or percent of the system soft/hard limit which is set for the
process (ulimit -n)

Syntax:

 IF OPEN-FILES <operator> <number> [%] THEN action

You can also check accumulated number for the process and all its children.
Note: The total open files test doesn't allow to set the limit as percent.
Syntax:

 IF TOTAL OPEN-FILES <operator> <number> THEN action
...
Example:
 ...
 check process myproc with pidfile /var/run/myproc.pid
    if open-files >= 90% then alert
    if open-files >= 99% then restart
    if total open-files > 5000 then alert
