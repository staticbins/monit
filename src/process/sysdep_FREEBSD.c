...
int initprocesstree_sysdep(ProcessTree_T **reference, ProcessEngine_Flags pflags) {
        ...
        if (! kvm_handle) {
                Log_error("system statistics error -- cannot initialize kvm interface\n");
                return 0;
        }
...
bool used_system_memory_sysdep(SystemInfo_T *si) {
        ...
        if (len != sizeof(unsigned int)) {
                Log_error("system statistics error -- active memory usage statics error\n");
                return false;
        }
        unsigned int wired;
        ...
        if (len != sizeof(unsigned int)) {
                Log_error("system statistics error -- wired memory usage statics error\n");
                return false;
        }
        ...
        if (sysctlbyname("kstat.zfs.misc.arcstats.size", &arcsize, &len, NULL, 0) == 0) {
                if (len != sizeof(arcsize)) {
                        Log_error("system statistics error -- ZFS ARC memory usage statics error\n");
                        return false;
                }
        }
