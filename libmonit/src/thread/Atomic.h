/*
 * Copyright (C) 2007-2019 Tildeslash Ltd. All rights reserved.
 ...
 *
 * This is a legacy interface for simple atomic operations and should be
 * removed when C11 _Atomic becomes available on all supported platforms.
 *
 * @file
 */
...
#define Atomic_inc(A)  (__atomic_add_fetch(&(&(A))->counter, 1, __ATOMIC_SEQ_CST))
#else
...
#elif ((__GNUC__ > 4) || ((__GNUC__ == 4) && (__GNUC_MINOR__ >= 7)))
#define Atomic_dec(A)  (__atomic_sub_fetch(&(&(A))->counter, 1, __ATOMIC_SEQ_CST))
#else
 ...
 */
#if HAVE_STDATOMIC_H
#define Atomic_cas(A, o, n) ({int __expectAtomic = (o); atomic_compare_exchange_strong(&(&(A))->counter, &__expectAtomic, (n));})
#elif DARWIN
...
#elif ((__GNUC__ > 4) || ((__GNUC__ == 4) && (__GNUC_MINOR__ >= 7)))
#define Atomic_cas(A, o, n) ({int __expectAtomic = (o); __atomic_compare_exchange_n(&(&(A))->counter, &__expectAtomic, (n), 0, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);})
#else
